      ...
      <form onSubmit={handleSearchSubmit} className="space-y-3">
        <div className="flex flex-col md:flex-row items-center gap-2.5">
          <div className="relative flex-1 w-full">
            <Search className="w-5 h-5 text-slate-500 dark:text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
            <input
              type="text"
              value={keyword}
              onChange={(e) => setKeyword(e.target.value)}
              placeholder={t.searchPlaceholder}
              className="w-full pl-11 pr-4 py-3 rounded-xl bg-white dark:bg-[#090d1f] border border-slate-300 dark:border-white/15 text-base font-medium text-slate-950 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 transition-all shadow-sm dark:shadow-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full md:w-auto px-7 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-base font-bold shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 shrink-0 cursor-pointer disabled:opacity-50 active:scale-95"
          >
            {loading ? (
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 text-xs">
          <span className="text-slate-800 dark:text-slate-200 font-bold">Quick queries:</span>
          {sampleKeywords.map((tag) => (
            <button
              key={tag}
              type="button"
              onClick={() => handleQuickQuery(tag)}
              className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-white/[0.08] hover:bg-indigo-50 dark:hover:bg-indigo-600/30 text-slate-800 dark:text-slate-100 hover:text-indigo-600 dark:hover:text-indigo-200 border border-slate-300 dark:border-white/15 hover:border-indigo-400 dark:hover:border-indigo-400/50 transition-all cursor-pointer font-semibold text-xs"
            >
              {tag}
            </button>
          ))}
          {keyword && (
            <button
              ...
              className="ml-auto text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white underline cursor-pointer text-xs font-semibold"
            >
              Clear
            </button>
          )}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 text-xs">
          <select
            ...
            className="px-3.5 py-2 rounded-lg bg-white dark:bg-[#090d1f] border border-slate-300 dark:border-white/15 text-slate-800 dark:text-slate-100 font-semibold focus:outline-none focus:border-indigo-500 cursor-pointer shadow-sm dark:shadow-none"
          >
          ...
          <button
            type="button"
            className="ml-auto px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-white/[0.04] hover:bg-slate-200 dark:hover:bg-white/[0.08] border border-slate-300 dark:border-white/[0.08] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white flex items-center gap-1.5 transition-all cursor-pointer shadow-sm dark:shadow-none"
          >
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center p-2.5 rounded-xl border transition-all text-center cursor-pointer ${
                isSelected
                  ? 'bg-indigo-600/10 dark:bg-indigo-600/20 border-indigo-500/40 dark:border-indigo-500/60 shadow-sm dark:shadow-md dark:shadow-indigo-500/20 ring-1 ring-indigo-500/30'
                  : 'bg-slate-50 dark:bg-white/[0.02] border-slate-200 dark:border-white/[0.05] hover:border-slate-300 dark:hover:border-white/20 hover:bg-slate-100 dark:hover:bg-white/[0.04]'
              }`}
            >
              ...
              <div className="text-xs font-bold text-slate-900 dark:text-white mt-0.5 tabular-nums">{p.count}</div>
            </button>
          );
        })}
      </div>
