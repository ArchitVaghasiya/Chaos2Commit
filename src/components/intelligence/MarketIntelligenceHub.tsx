  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 border-slate-200 dark:border-white/[0.06] shadow-xl">
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200 dark:border-white/[0.06]">
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                  isSelected
                    ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-slate-900 dark:text-white shadow-md shadow-purple-600/30 font-bold'
                    : 'bg-white/[0.02] text-slate-600 dark:text-slate-300 hover:bg-white/[0.05] border border-slate-200 dark:border-white/[0.04]'
                }`}
              >
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="glass-card p-5 border-slate-200 dark:border-white/[0.06] shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4 text-xs">
              <div className="p-3 rounded-xl bg-white/[0.02] border border-slate-200 dark:border-white/[0.04]">
                <div className="text-[10px] text-slate-600">Total Capital Raised</div>
                <div className="text-xl font-extrabold text-slate-900 dark:text-white mt-0.5">{selectedCompany.funding.totalRaised}</div>
              </div>

              <div className="p-3 rounded-xl bg-white/[0.02] border border-slate-200 dark:border-white/[0.04]">
                <div className="text-[10px] text-slate-600">Last Round Announced</div>
                <div className="text-sm font-bold text-slate-700 dark:text-slate-200 mt-1">{selectedCompany.funding.lastRoundDate}</div>
              </div>
            </div>

            <div>
              <div className="text-[11px] font-semibold text-slate-600 mb-1.5">Backing Investors:</div>
              <div className="flex flex-wrap gap-1.5">
                {selectedCompany.funding.leadInvestors.map((inv, idx) => (
                  <span key={idx} className="px-2.5 py-1 rounded-lg text-[10px] font-medium bg-white/[0.04] text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-white/[0.08]">
                    {inv}
                  </span>
                ))}
              </div>
          ...
          <div className="glass-card p-5 border-slate-200 dark:border-white/[0.06] shadow-xl">
            ...
            <div className="space-y-2 mb-4">
              <div className="text-[11px] font-semibold text-slate-600">Growth Departments:</div>
              <div className="space-y-1.5 text-xs">
                {selectedCompany.hiring.highGrowthDepartments.map((dept, i) => (
                  <div key={i} className="p-2 rounded-lg bg-white/[0.02] border border-slate-200 dark:border-white/[0.04] text-slate-700 dark:text-slate-200 flex items-center justify-between">
                    <span>{dept}</span>
            ...
            <div>
              <div className="text-[11px] font-semibold text-slate-600 mb-1.5">Recent Key Executive Appointments:</div>
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="glass-card p-5 border-slate-200 dark:border-white/[0.06] shadow-xl">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-200 dark:border-white/[0.06]">
              ...
              <span className="text-[10px] text-slate-600">Verified via Public DNS &amp; Job Specs</span>
            </div>

            <div className="space-y-3">
              {selectedCompany.techStack.map((cat, i) => (
                <div key={i} className="p-3 rounded-xl bg-white/[0.02] border border-slate-200 dark:border-white/[0.04]">
                  <div className="text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1.5">{cat.category}</div>
                  <div className="flex flex-wrap gap-1.5">
          ...
          <div className="glass-card p-5 border-slate-200 dark:border-white/[0.06] shadow-xl">
            ...
            <div>
              <div className="text-[11px] font-semibold text-slate-600 mb-1">Primary Incumbent Vendors:</div>
              <div className="flex flex-wrap gap-1.5">
                {selectedCompany.competitorInsights.topCompetitors.map((comp, i) => (
                  <span key={i} className="px-2 py-0.5 rounded text-[11px] text-slate-600 dark:text-slate-300 bg-white/[0.04] border border-slate-200 dark:border-white/[0.06]">
                    {comp}
                  </span>
                ))}
              </div>
